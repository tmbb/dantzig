defmodule Dantzig.DSL.DynamicVariableAccessTest do
  @moduledoc """
  Test dynamic variable access macros like queen2d[i, :_].
  """
  use ExUnit.Case, async: true

  require Dantzig.Problem.DSL, as: DSL

  test "dynamic variable access macro works" do
    # Test that we can create dynamic macros for variable names
    # This should allow queen2d[i, :_] to work

    # For now, let's test with a macro that can handle any variable name
    queen2d_expr = DSL.dynamic_var_access(:queen2d, [1, :_])
  ...
  test "dynamic variable access with different names" do
    # Test with different variable names
    queen3d_expr = DSL.dynamic_var_access(:queen3d, [1, 2, 3])
    qty_expr = DSL.dynamic_var_access(:qty, [:food])
  ...
  test "dynamic variable access with wildcards" do
    # Test with wildcards
    wildcard_expr = DSL.dynamic_var_access(:queen2d, [:_, :_])
  ...
  test "dynamic variable access in constraint context" do
    # Test queen2d[i, :_] == 1
    constraint_ast = {:==, [], [DSL.dynamic_var_access(:queen2d, [1, :_]), 1]}
  ...
  test "dynamic variable access in sum context" do
    # Test sum(queen2d[:_, :_])
    sum_expr = {:sum, [], [DSL.dynamic_var_access(:queen2d, [:_, :_])]}
  ...
  test "dynamic variable access with mixed syntax" do
    # Test that we can mix different variable names
    queen2d_expr = DSL.dynamic_var_access(:queen2d, [1, :_])
    queen3d_expr = DSL.dynamic_var_access(:queen3d, [:_, 2, :_])
    qty_expr = DSL.dynamic_var_access(:qty, [:food])
